...
{
    public class TestTicketMgmt : BusinessTest<Model>
    {
        ...
        protected override void RegisterServices()
        {
            ...
            ServiceCollection?.AddScoped<ITicketMgmtBus, TicketMgmtBus>();
        }
        ...
        protected async Task TestGetFullNameByIdAsync(MDtoRequestFindByInt dto)
        {
            ...
            {
                var bus = GetService<ITicketMgmtBus>();
                if (bus == null) return;
        ...
        protected async Task TestSaveRangeTransactionAsync(TicketSaveRangeDtoRequest? request)
        {
            await RunTest("TestSaveRangeAsync", async (log) =>
            {
                var createDtos = request?.CreateData;
