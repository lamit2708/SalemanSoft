...
namespace VSoft.Company.USR.User.Business.UnitTest.Bases
{
    public class TestUserMgmt : BusinessTest<Model>
    {
        ...
        protected override void RegisterServices()
        {
            ...
            ServiceCollection?.AddScoped<IUserMgmtBus, UserMgmtBus>();
        }
        ...
        protected async Task TestGetFullNameByIdAsync(MDtoRequestFindByLong dto)
        {
            ...
            {
                var bus = GetService<IUserMgmtBus>();
                if (bus == null) return;
        ...
        protected async Task TestSaveRangeTransactionAsync(UserSaveRangeDtoRequest? request)
        {
            await RunTest("TestSaveRangeAsync", async (log) =>
            {
                var createDtos = request?.CreateData;
