...
{
    public class TestUserCustomerMgmt : BusinessTest<Model>
    {
        ...
        protected override void RegisterServices()
        {
            ...
            ServiceCollection?.AddScoped<IUserCustomerMgmtBus, UserCustomerMgmtBus>();
        }
        ...
        protected async Task TestGetFullNameByIdAsync(MDtoRequestFindByLong dto)
        {
            ...
            {
                var bus = GetService<IUserCustomerMgmtBus>();
                if (bus == null) return;
        ...
        protected async Task TestSaveRangeTransactionAsync(UserCustomerSaveRangeDtoRequest? request)
        {
            await RunTest("TestSaveRangeAsync", async (log) =>
            {
                var createDtos = request?.CreateData;
