...
{
    public class TestCustomerMgmt : BusinessTest<Model>
    {
        ...
        protected override void RegisterServices()
        {
            ...
            ServiceCollection?.AddScoped<ICustomerMgmtBus, CustomerMgmtBus>();
        }
        ...
        protected async Task TestGetFullNameByIdAsync(MDtoRequestFindByInt dto)
        {
            ...
            {
                var bus = GetService<ICustomerMgmtBus>();
                if (bus == null) return;
        ...
        protected async Task TestSaveRangeTransactionAsync(CustomerSaveRangeDtoRequest? request)
        {
            await RunTest("TestSaveRangeAsync", async (log) =>
            {
                var createDtos = request?.CreateData;
