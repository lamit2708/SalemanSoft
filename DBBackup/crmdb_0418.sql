...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 18, 2023 at 12:17 PM
...
('20230418093331_Init', '7.0.2'),
('20230418094627_Init', '7.0.2'),
('20230418094734_JoinCustomerInfo', '7.0.2');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vdealtag`  AS SELECT `d`.`Id` AS `Id`, `d`.`Name` AS `DealName`, `c`.`Name` AS `CustomerName`, `c`.`Phone` AS `CustomerPhone`, `d`.`PridictPrice` AS `PridictPrice`, `d`.`DealStepId` AS `DealStepId` FROM (`deal` `d` join `customer` `c`) WHERE `d`.`CustomerId` = `c`.`Id``Id`  ;
